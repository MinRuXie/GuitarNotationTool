...
$(function(){
    ...
    $('.chord').each(function(index){
        for (let i=0; i <= grid_count - 1; i++) {
            let grid = `
                <div class="grid" data-chord="${index+1}" data-grid="${i}" data-note="${notes_array_2[index][i]}" value="${notes_array[index][i]}" title="${notes_array[index][i]}">
                    <span class="note-${notes_array_3[index][i]}">${i}</span>
                </div>`;
            $(this).append(grid);
        }
    });
